...
class ColumnSwitchAttack(BaseAttack):
    ...
    def fit(
        ...
        if dataset.numerical_features is None:
            raise ValueError(
                f"This dataset ({dataset}) has the numerical_features attribute not set, which is needed for a ColumnSwitchAttack!"
            )
        ...
        if numerical_only:
            ...
            ]  # TODO: get from feature mapping
        else:
            feature_indexes = list(range(len(dataset.features.feature_names_model)))
        console.print(
            f"[bold #ed1cdf][CSA][/] [#f7c8f3] Fitting on features: {feature_indexes} (aka. {[dataset.features.feature_names_model[f] for f in feature_indexes]})"
        )

        top_score = np.inf
        top_combi: list[
            int
        ] = []  # combi means lsit in index with data switching to the right. E.g. [1,4,6] would result in 1->4->6->1, so column 1 now has data from column 6, column 4 has now date from column 1 and column 6 has now data from column 4.
        ...
        try:
            ...
            if max_tries is not None:
                task = progress.add_task(
                    "[bold #ed1cdf][CSA][/] [#f7c8f3] Fitting MAX...",
                    total=max_tries,
                    combi="",
                )
                progress.start()
                for _ in range(max_tries):
                    combi = np.random.choice(
                        feature_indexes, size=(n_switches,), replace=False
                    )
                ...
                task = progress.add_task(
                    "[bold #ed1cdf][CSA][/] [#f7c8f3] Fitting ALL...",
                    total=math.factorial(len(feature_indexes))
                    / math.factorial(len(feature_indexes) - n_switches),
                    combi="",
                )
                progress.start()
    ...
    def _generate(self, x: np.ndarray) -> np.ndarray:
        if self.top_combi is None:
            raise RuntimeError(
                "Fit the attack first before generating adverserial data."
            )
        switched = self._switch_columns(x, self.top_combi)
        valid, _ = self.is_attack_valid(
            np.expand_dims(x, axis=0), np.expand_dims(switched, axis=0), self.epsilon
        )
        return switched if valid else x


class ColumnSwitchAttackLocal(BaseAttack):
    def __init__(
        self,
        model: BaseModel,
        task: Literal["classification", "regression"],
